module KwalifyToJsonSchema
  class Cli < Thor
    ...
    method_option :issues_to_description => :boolean, :default => false, :desc => "Will append any conversion issue to the schema description"

    def convert(kwalify_schema_file, result_file)
      options = {
        issues_to_description: issues_to_description?,
      }
      KwalifyToJsonSchema.convert_file(kwalify_schema_file, result_file, options)
    end
